...
const missionCounter = async function (): Promise<any> {
  ...
  const aBuildings: Array<UserBuildings> = JSON.parse(localStorage.aBuildings).value;
  ...
  socket.on("departmentBuy", async () => {
    ...
    const aBuildings = JSON.parse(localStorage.aBuildings).value as Array<UserBuildings>;
